...
class ExposeGenerator:
    ...
    def template(self, text, key, value):
        ...
        escaped_value = (
            collapsed_value.replace("\\", "\\\\").replace("/", "\\/").replace("&", "\\&")
        )
    ...
    def run_command(self, cmd, capture=True, check=False):
        ...
        try:
            result = subprocess.run(
                cmd, shell=isinstance(cmd, str), capture_output=capture, text=True, check=check
            )
    ...
    def read_files(self):
        ...
        for i, path in enumerate(self.paths):
            ...
            for file_path in files:
                ...
                if (
                    self.config["autorotate"]
                    and orientation
                    and orientation.isdigit()
                    and 5 <= int(orientation) <= 8
                ):
    ...
    def _encode_h264(
        self, filepath, output, res, mbit, mbitmax, filters_arg, filters_full, audio_args, firstpass
    ):
...
def main():
    ...
    parser.add_argument(
        "-d", "--draft", action="store_true", help="Draft mode: single resolution, fast encoding"
    )
